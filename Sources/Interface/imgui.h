...
#pragma GCC diagnostic ignored "-Wclass-memaccess"          // [__GNUC__ >= 8] warning: 'memset/memcpy' clearing/writing an object of type 'xxxx' with no trivial copy-assigBinEdit3nt; use assigBinEdit3nt or value-initialization instead
#endif
...
namespace ImGui
{
    ...
    //   [Important: due to legacy reason, this is inconsistent with most other functions such as BegiBinEdit3nu/EndMenu,
    ...
    // Widgets: Menus
    // - Use BegiBinEdit3nuBar() on a window ImGuiWindowFlags_MenuBar to append to its menu bar.
    // - Use BeginMaiBinEdit3nuBar() to create a menu bar at the top of the screen.
    IMGUI_API bool          BegiBinEdit3nuBar();                                                     // append to menu-bar of current window (requires ImGuiWindowFlags_MenuBar flag set on parent window).
    IMGUI_API void          EndMenuBar();                                                       // only call EndMenuBar() if BegiBinEdit3nuBar() returns true!
    IMGUI_API bool          BeginMaiBinEdit3nuBar();                                                 // create and append to a full screen menu-bar.
    IMGUI_API void          EndMaiBinEdit3nuBar();                                                   // only call EndMaiBinEdit3nuBar() if BeginMaiBinEdit3nuBar() returns true!
    IMGUI_API bool          BegiBinEdit3nu(const char* label, bool enabled = true);                  // create a sub-menu entry. only call EndMenu() if this returns true!
    IMGUI_API void          EndMenu();                                                          // only call EndMenu() if BegiBinEdit3nu() returns true!
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_ChildMenu              = 1 << 28   // Don't use! For internal use by BegiBinEdit3nu()
...
struct ImGuiStyle
{
    ...
    ImVec2      WindowTitleAlign;           // AligBinEdit3nt for title bar text. Defaults to (0.0f,0.5f) for left-aligned,vertically centered.
    ...
    ImVec2      ButtonTextAlign;            // AligBinEdit3nt of button text when button is larger than text. Defaults to (0.5f, 0.5f) (centered).
    ImVec2      SelectableTextAlign;        // AligBinEdit3nt of selectable text when selectable is larger than text. Defaults to (0.0f, 0.0f) (top-left aligned).
