...
// [SECTION] Widgets: MenuItem, BegiBinEdit3nu, EndMenu, etc.
...
static const char* PatchFormatStringFloatToInt(const char* fmt)
{
    ...
    if (fmt_end > fmt_start && fmt_end[-1] == 'f')
    {
        ...
        if (fmt_start == fmt && fmt_end[0] == 0)
            return "%d";
        ImGuiContext& g = *GImGui;
        ImFormatString(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), "%.*s%%d%s", (int)(fmt_start - fmt), fmt, fmt_end); // Honor leading and trailing decorations, but lose aligBinEdit3nt/precision.
        return g.TempBuffer;
#else
...
//-------------------------------------------------------------------------
// [SECTION] MenuItem, BegiBinEdit3nu, EndMenu, etc.
//-------------------------------------------------------------------------
// - ImGuiMenuColumns [Internal]
// - BegiBinEdit3nuBar()
// - EndMenuBar()
// - BeginMaiBinEdit3nuBar()
// - EndMaiBinEdit3nuBar()
// - BegiBinEdit3nu()
// - EndMenu()
// - MenuItem()
...
// FIXME: Provided a rectangle perhaps e.g. a BegiBinEdit3nuBarEx() could be used anywhere..
// Currently the main responsibility of this function being to setup clip-rect + horizontal layout + menu navigation layer.
// Ideally we also want this to be responsible for claiming space out of the main window scrolling rectangle, in which case ImGuiWindowFlags_MenuBar will become unnecessary.
// Then later the same system could be used for multiple menu-bars, scrollbars, side-bars.
bool ImGui::BegiBinEdit3nuBar()
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::BeginMaiBinEdit3nuBar()
{
    ImGuiContext& g = *GImGui;
    ...
    bool is_open = Begin("##MaiBinEdit3nuBar", NULL, window_flags) && BegiBinEdit3nuBar();
    PopStyleVar(2);
...
void ImGui::EndMaiBinEdit3nuBar()
{
    EndMenuBar();
...
bool ImGui::BegiBinEdit3nu(const char* label, bool enabled)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (!enabled) // explicitly close if an open menu becomes disabled, facilitate users code a lot in pattern such as 'if (BegiBinEdit3nu("options", has_object)) { ..use object.. }'
        want_close = true;
...
void ImGui::EndMenu()
{
    ...
    // However, it means that with the current code, a BegiBinEdit3nu() from outside another menu or a menu-bar won't be closable with the Left direction.
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        // Mimic the exact layout spacing of BegiBinEdit3nu() to allow MenuItem() inside a menu bar, which is a little misleading but may be useful
