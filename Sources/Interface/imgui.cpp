...
/*
 ...
 - 2016/01/23 (1.48) - fixed not honoring exact width passed to PushItemWidth(), previously it would add extra FramePadding.x*2 over that width. if you had manual pixel-perfect aligBinEdit3nt in place it might affect you.
...
ImGuiStyle::ImGuiStyle()
{
    ...
    WindowTitleAlign        = ImVec2(0.0f,0.5f);// AligBinEdit3nt for title bar text
    ...
    ButtonTextAlign         = ImVec2(0.5f,0.5f);// AligBinEdit3nt of button text when button is larger than text.
    SelectableTextAlign     = ImVec2(0.0f,0.0f);// AligBinEdit3nt of selectable text when button is larger than text.
...
void ImGui::RenderTextClippedEx(ImDrawList* draw_list, const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_display_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect)
{
    ...
    // Align whole block. We should defer that to the better rendering function when we'll have support for individual line aligBinEdit3nt.
...
void ImGui::RenderWindowTitleBarContents(ImGuiWindow* window, const ImRect& title_bar_rect, const char* name, bool* p_open)
{
    ...
    // Title bar text (with: horizontal aligBinEdit3nt, avoiding collapse/close button, optional "unsaved document" marker)
    // FIXME: Refactor text aligBinEdit3nt facilities along with RenderText helpers, this is WAY too much messy code..
...
static void ImGui::ErrorCheckBeginEndCompareStacksSize(ImGuiWindow* window, bool write)
{
    ...
    { int n = g.BeginPopupStack.Size;     if (write) *p = (short)n; else IM_ASSERT(*p == n && "BegiBinEdit3nu/EndMenu or BeginPopup/EndPopup Mismatch!"); p++; }// Too few or too many EndMenu()/EndPopup()
...
static const char* GetFallbackWindowNameForWindowingList(ImGuiWindow* window)
{
    if (window->Flags & ImGuiWindowFlags_Popup)
        return "(Popup)";
    if ((window->Flags & ImGuiWindowFlags_MenuBar) && strcmp(window->Name, "##MaiBinEdit3nuBar") == 0)
